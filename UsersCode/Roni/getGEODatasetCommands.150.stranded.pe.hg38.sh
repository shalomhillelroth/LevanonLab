...
echo "python /home/alu/hillelr/scripts/GGPS/Session/PipelineManger.py -c /home/alu/fulther/ConfigsHillel/downloadFixup.conf -d \$WANTED_DIR/RawData -t 0,2 -f fastq -o \$WANTED_DIR/RawData -l \$WANTED_DIR/RawData/Logs --no_extensions>> \$WANTED_DIR/RawData/Logs/deduperAndFastQC_Log.txt" >> $WANTED_DIR/commands$SUFFIX.sh
...
echo "#python /home/alu/fulther/Scripts/scripts_python/General/SalmonQuantification.py -i \$WANTED_DIR/RawData/RemovedDupsFASTQ\$SUFFIX -o \$WANTED_DIR/Salmon\$SUFFIX -t 0,2 -f fastq.lzma > \$WANTED_DIR/Salmon\$SUFFIX/runSalmon_Log.txt" >> $WANTED_DIR/commands$SUFFIX.sh
echo "python /home/alu/hillelr/scripts/GGPS/Session/PipelineManger.py -c /private/common/Software/PipelineConfigs/runSalmon_1.4.0.conf -d \$WANTED_DIR/RawData/RemovedDupsFASTQ\$SUFFIX -t 0,2 -f fastq.lzma -o \$WANTED_DIR/Salmon_1.4.0\$SUFFIX -l \$WANTED_DIR/Salmon_1.4.0\$SUFFIX -a transcripts_index=\'/private/dropbox/Salmon_1.4.0/salmon_index/hg38\' tx2id_geneMap=\'/private/dropbox/Salmon_1.4.0/salmon_index/hg38/gencode_v32.transcriptToGeneID.tab\' >> \$WANTED_DIR/Salmon_1.4.0\$SUFFIX/runSalmon_Log.txt" >> $WANTED_DIR/commands$SUFFIX.sh
...
echo "RNAEditingIndex1.1 -d \$WANTED_DIR/STAR\$SUFFIX -l \$WANTED_DIR/AluEditingIndex\$SUFFIX -o \$WANTED_DIR/AluEditingIndex\$SUFFIX -os \$WANTED_DIR/AluEditingIndex\$SUFFIX --genome hg38 --stranded --paired_end >> \$WANTED_DIR/AluEditingIndex\$SUFFIX/runAluIndex_Log.txt" >> $WANTED_DIR/commands$SUFFIX.sh
echo -e "\n" >> $WANTED_DIR/commands$SUFFIX.sh
...
echo "python /home/alu/hillelr/scripts/GGPS/Tools/HyperEditing/HyperEditingRunner.py -d  \$WANTED_DIR/STAR\$SUFFIX -o \$WANTED_DIR/HE\$SUFFIX -os \$WANTED_DIR/HE\$SUFFIX/summary -so \$WANTED_DIR/STAR\$SUFFIX --no_recurring_reads -f lzma -mp 'Unmapped.out.mate' -u lzcat -pe -se --keep_unmapped_BAMs >> \$WANTED_DIR/HE\$SUFFIX/runHE_Log.txt" >> $WANTED_DIR/commands$SUFFIX.sh
echo "# HE stats command" >> $WANTED_DIR/commands$SUFFIX.sh
echo "python /home/alu/fulther/Scripts/scripts_python/General/AnalyzeStatisticsHE.py -d \$WANTED_DIR/HE\$SUFFIX -i PE_0.05_0.6_30_0.6_0.1_0.8_0.2 -o \$WANTED_DIR/summary/HE\$SUFFIX -po \$WANTED_DIR/summary/plots/HE\$SUFFIX" >> $WANTED_DIR/commands$SUFFIX.sh
echo -e "\n" >> $WANTED_DIR/commands$SUFFIX.sh
echo "# stranded correction" >> $WANTED_DIR/commands$SUFFIX.sh
echo "# 1 2 0: PE, 1st antisense 2ed sense, run as real mode (1=PE as SE)" >> $WANTED_DIR/commands$SUFFIX.sh
echo "paste <(ls \$WANTED_DIR/HE\$SUFFIX/AnalyseMM | sed 's/analyseMM/fastq.gz/g') <(ls \$WANTED_DIR/HE\$SUFFIX/AnalyseMM | sed 's/-..analyseMM//g') > \$WANTED_DIR/HE\$SUFFIX/inputs_outputs_PE.tab" >> $WANTED_DIR/commands$SUFFIX.sh
echo "/home/alu/hagitpt/scripts/hyper_editing_scripts/stranded_sample.sh \$WANTED_DIR/HE\$SUFFIX/inputs_outputs_PE.tab 1 2 0 \$WANTED_DIR/HE\$SUFFIX 2 \"0.05 0.6 30 0.6 0.1 0.8 0.2\" > \$WANTED_DIR/HE\$SUFFIX/runHE_Log.stranded.txt" >> $WANTED_DIR/commands$SUFFIX.sh
...
echo "Rscript /home/alu/fulther/Scripts/scripts_R/General/ZMAD_ScoreCalc_V2.R -i \$WANTED_DIR/Salmon_1.4.0\$SUFFIX/SalmonTPM.wideSamples.csv -o \$WANTED_DIR/ISG\$SUFFIX -p ArticleCalcLog2 --log2" >> $WANTED_DIR/commands$SUFFIX.sh
echo -e "\n" >> $WANTED_DIR/commands$SUFFIX.sh
