...
echo "RNAEditingIndex1.1 -d \$WANTED_DIR/STAR\$SUFFIX -l \$WANTED_DIR/AluEditingIndex\$SUFFIX -o \$WANTED_DIR/AluEditingIndex\$SUFFIX -os \$WANTED_DIR/AluEditingIndex\$SUFFIX --genome mm10 --stranded --paired_end >> \$WANTED_DIR/AluEditingIndex\$SUFFIX/runAluIndex_Log.txt" >> $WANTED_DIR/commands$SUFFIX.sh
echo -e "\n" >> $WANTED_DIR/commands$SUFFIX.sh
...
echo "[ ! -d \$WANTED_DIR/HE\$SUFFIX ] && mkdir -p \$WANTED_DIR/HE\$SUFFIX" >> $WANTED_DIR/commands$SUFFIX.sh
echo "# Run HE command: notice genome" >> $WANTED_DIR/commands$SUFFIX.sh
echo "python /home/alu/hillelr/scripts/GGPS/Tools/HyperEditing/HyperEditingRunner.py -d  \$WANTED_DIR/STAR\$SUFFIX -o \$WANTED_DIR/HE\$SUFFIX -os \$WANTED_DIR/HE\$SUFFIX/summary -so \$WANTED_DIR/STAR\$SUFFIX --no_recurring_reads -f lzma -mp 'Unmapped.out.mate' -u lzcat -pe -se --keep_unmapped_BAMs -gf /private/dropbox/Genomes/Mouse/mm10/all.fa -gi /private/dropbox/Genomes/Mouse/mm10/all -gti /private/dropbox/Genomes/Mouse/mm10/all>> \$WANTED_DIR/HE\$SUFFIX/runHE_Log.txt" >> $WANTED_DIR/commands$SUFFIX.sh
echo "# HE stats command" >> $WANTED_DIR/commands$SUFFIX.sh
